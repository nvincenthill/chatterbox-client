...
// create room list and add to DOM
let addRoomsToSelect = () => {
  let filtered = messages.results.filter(a => a.roomname);
  for (let prop of filtered) {
    if (!rooms.includes(prop.roomname)) {
...
// create a friend and add to DOM
let createFriend = (friend) => {
  if ($('#nofriends') !== undefined) {
    $('#nofriends').remove();
  }
